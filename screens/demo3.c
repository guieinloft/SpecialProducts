...
bool screens_demo3_loadmedia(Game *game, struct media_t *media, struct variables_t *var) {
    ...
    if(!texture_load_from_text(media->tex_text[0][1], game_get_renderer(game), media->font, "a", COLOR_DBLUE)) return false;
    if(!texture_load_from_text(media->tex_text[0][2], game_get_renderer(game), media->font, "+b)(", COLOR_TEXT_DEFAULT_LIGHT)) return false;
    if(!texture_load_from_text(media->tex_text[0][3], game_get_renderer(game), media->font, "a", COLOR_DBLUE)) return false;
    if(!texture_load_from_text(media->tex_text[0][4], game_get_renderer(game), media->font, "-b) = ", COLOR_TEXT_DEFAULT_LIGHT)) return false;
    if(!texture_load_from_text(media->tex_text[0][5], game_get_renderer(game), media->font, "a²", COLOR_DBLUE)) return false;

    if(!texture_load_from_text(media->tex_text[1][0], game_get_renderer(game), media->font, "(", COLOR_TEXT_DEFAULT_LIGHT)) return false;
    if(!texture_load_from_text(media->tex_text[1][1], game_get_renderer(game), media->font, "a", COLOR_DBLUE)) return false;
    if(!texture_load_from_text(media->tex_text[1][2], game_get_renderer(game), media->font, "+b)(a", COLOR_TEXT_DEFAULT_LIGHT)) return false;
    if(!texture_load_from_text(media->tex_text[1][3], game_get_renderer(game), media->font, "-b", COLOR_DBLUE)) return false;
    if(!texture_load_from_text(media->tex_text[1][4], game_get_renderer(game), media->font, ") = a²", COLOR_TEXT_DEFAULT_LIGHT)) return false;
    if(!texture_load_from_text(media->tex_text[1][5], game_get_renderer(game), media->font, "-ab", COLOR_DBLUE)) return false;

    if(!texture_load_from_text(media->tex_text[2][0], game_get_renderer(game), media->font, "(a+", COLOR_TEXT_DEFAULT_LIGHT)) return false;
    if(!texture_load_from_text(media->tex_text[2][1], game_get_renderer(game), media->font, "b", COLOR_DBLUE)) return false;
    if(!texture_load_from_text(media->tex_text[2][2], game_get_renderer(game), media->font, ")(", COLOR_TEXT_DEFAULT_LIGHT)) return false;
    if(!texture_load_from_text(media->tex_text[2][3], game_get_renderer(game), media->font, "a", COLOR_DBLUE)) return false;
    ...
    if(!texture_load_from_text(media->tex_text[2][5], game_get_renderer(game), media->font, "+ba", COLOR_DBLUE)) return false;

    if(!texture_load_from_text(media->tex_text[3][0], game_get_renderer(game), media->font, "(a+", COLOR_TEXT_DEFAULT_LIGHT)) return false;
    if(!texture_load_from_text(media->tex_text[3][1], game_get_renderer(game), media->font, "b", COLOR_DBLUE)) return false;
    if(!texture_load_from_text(media->tex_text[3][2], game_get_renderer(game), media->font, ")(a", COLOR_TEXT_DEFAULT_LIGHT)) return false;
    if(!texture_load_from_text(media->tex_text[3][3], game_get_renderer(game), media->font, "-b", COLOR_DBLUE)) return false;
    ...
    if(!texture_load_from_text(media->tex_text[3][5], game_get_renderer(game), media->font, "-b²", COLOR_DBLUE)) return false;
    ...
    if(!texture_load_from_text(media->tex_text[4][4], game_get_renderer(game), media->font, "-b²", COLOR_TEXT_DEFAULT_LIGHT)) return false;
    if(!texture_load_from_text(media->tex_text[4][5], game_get_renderer(game), media->font, " ", COLOR_DBLUE)) return false;
...
bool screens_demo3_loadobjects(Game *game, struct objects_t *objects, struct media_t *media, struct variables_t *var) {
    objects->balloon = balloon_create(media->tex_balloon, media->tex_anim, "txt/demo3_1.txt", true);
    if (objects->balloon == NULL) return false;
    return true;
}

void screens_demo3_loadvariables(Game *game, struct variables_t *var) {
    var->next = false;
    var->ret = SCREEN_NEXT;
...
Screen screens_demo3_close(struct media_t *media, struct objects_t *objects, struct variables_t *var) {
...
void screens_demo3_drawbg(Game *game, struct media_t *media, struct variables_t *var) {
    ...
    SDL_RenderDrawLine(game_get_renderer(game), 28, 81, 28, 222);
    SDL_RenderDrawLine(game_get_renderer(game), 24, 81, 28, 81);
    SDL_RenderDrawLine(game_get_renderer(game), 24, 222, 28, 222);

    SDL_RenderDrawLine(game_get_renderer(game), 16, 81, 16, 126);
    SDL_RenderDrawLine(game_get_renderer(game), 12, 81, 16, 81);
    SDL_RenderDrawLine(game_get_renderer(game), 12, 126, 16, 126);

    //draw horizontal lines
    SDL_SetRenderDrawColor(game_get_renderer(game), COLOR_DBLACK.r, COLOR_DBLACK.g, COLOR_DBLACK.b, COLOR_DBLACK.a);
    SDL_RenderDrawLine(game_get_renderer(game), 33, 227, 174, 227);
    SDL_RenderDrawLine(game_get_renderer(game), 33, 227, 33, 231);
    SDL_RenderDrawLine(game_get_renderer(game), 174, 227, 174, 231);

    SDL_RenderDrawLine(game_get_renderer(game), 177, 227, 222, 227);
    SDL_RenderDrawLine(game_get_renderer(game), 177, 227, 177, 231);
    SDL_RenderDrawLine(game_get_renderer(game), 222, 227, 222, 231);

    //draw letters
    texture_render(media->tex_letters[1], game_get_renderer(game), 8, 100, NULL);
    texture_render(media->tex_letters[0], game_get_renderer(game), 20, 148, NULL);
    texture_render(media->tex_letters[0], game_get_renderer(game), 100, 228, NULL);
    texture_render(media->tex_letters[1], game_get_renderer(game), 196, 228, NULL);

    //reset render target (NEVER FORGET)
    game_reset_render_target(game);
}

void screens_demo3_drawsquares(Game *game, struct media_t *media, struct variables_t *var) {
    //set canvas as render target
    texture_set_as_render_target(media->tex_canvas, game_get_renderer(game));
    //draw rect aa
    SDL_Rect rect_aa = {32, 128, 144, 96};
    ...
    //draw rect ba
    SDL_Rect rect_ba = {176, 128, 48, 96};
    ...
    //draw rect bb
    SDL_Rect rect_bb = {176, 80, 48, 48};
    if (var->text_number < 3) {
        SDL_SetRenderDrawColor(game_get_renderer(game), COLOR_DCYAN.r, COLOR_DCYAN.g, COLOR_DCYAN.b, (255 - 255 * var->text_offset / 8 * (var->text_number == 2)) * (var->text_number >= 2));
    }
    else {
        SDL_SetRenderDrawColor(game_get_renderer(game), COLOR_DRED.r, COLOR_DRED.g, COLOR_DRED.b, (255 * var->text_offset / 8 * (var->text_number == 3)) * (var->text_number >= 3));
    }
...
void screens_demo3_intro(Game *game, struct media_t *media, struct objects_t *objects, struct variables_t *var) {
    while (!var->next) {
        ...
        //draw canvas
        screens_demo3_drawbg(game, media, var);
...
void screens_demo3_demo(Game *game, struct media_t *media, struct objects_t *objects, struct variables_t *var) {
    while (!var->next) {
        while (SDL_PollEvent(&var->e) != 0) {
            if (var->e.type == SDL_QUIT) {
                var->next = true;
        ...
        //draw canvas
        screens_demo3_drawbg(game, media, var);
        screens_demo3_drawsquares(game, media, var);
        texture_render(media->tex_canvas, game_get_renderer(game), (SCREEN_W - texture_getw(media->tex_canvas))/2, 32, NULL);
...
void screens_demo3_end(Game *game, struct media_t *media, struct objects_t *objects, struct variables_t *var) {
...
Screen screens_demo3(Game *game) {
    ...
    screens_demo3_loadvariables(game, var);
    if (!screens_demo3_loadmedia(game, media, var)) {
        printf("Couldnt open textures\n");
        return SCREEN_ERROR;
    }
    if (!screens_demo3_loadobjects(game, objects, media, var)) {
    ...
    screens_demo3_intro(game, media, objects, var);

    var->next = false;
    if (var->ret == SCREEN_NEXT) {
        screens_demo3_demo(game, media, objects, var);
    }

    var->next = false;
    if (var->ret == SCREEN_NEXT) {
        if (!balloon_load_new_file(objects->balloon, "txt/demo3_2.txt", true)) {
            printf("Couldnt load new file\n");
            return SCREEN_ERROR;
        }
        screens_demo3_end(game, media, objects, var);
    }
    ...
    return screens_demo3_close(media, objects, var);
}
