...
Question *quiz2_question_n_create(int n);
Question *quiz2_question_1_7_create(bool inverted);
Question *quiz2_question_2_8_create(bool inverted);
Question *quiz2_question_3_9_create(bool inverted);
Question *quiz2_question_4_10_create(bool inverted);
Question *quiz2_question_5_11_create(bool inverted);
Question *quiz2_question_6_12_create(bool inverted);
Question *quiz2_question_13_create();

bool screens_quiz2_loadmedia(Game *game, struct media_t *media, struct variables_t *var) {
...
bool screens_quiz2_loadobjects(Game *game, struct objects_t *objects, struct media_t *media, struct variables_t *var) {
    //create question containers
    for (int i = 0; i < 13; i++) {
        objects->questions[i] = quiz2_question_n_create(i);
...
void screens_quiz2_loadvariables(Game *game, struct variables_t *var) {
    var->next = false;
    for (int i = 0; i < 12; i++) {
        var->correct[i] = false;
...
Screen screens_quiz2_close(struct media_t *media, struct objects_t *objects, struct variables_t *var) {
    for (int i = 0; i < 13; i++) {
...
void screens_quiz2_quiz(Game *game, struct media_t *media, struct objects_t *objects, struct variables_t *var) {
...
void screens_quiz2_results(Game *game, struct media_t *media, struct objects_t *objects, struct variables_t *var) {
    int timer = 120;
...
Screen screens_quiz2(Game *game) {
    ...
    screens_quiz2_loadvariables(game, var);
    if (!screens_quiz2_loadmedia(game, media, var)) {
        printf("Couldnt open textures\n");
        return SCREEN_ERROR;
    }
    if (!screens_quiz2_loadobjects(game, objects, media, var)) {
    ...
    screens_quiz2_quiz(game, media, objects, var);
    ...
    if (var->ret == SCREEN_NEXT) {
        screens_quiz2_results(game, media, objects, var);
    }
    game_set_points(game, POINTS_2, var->current_points);
    if (var->ret == SCREEN_NEXT) var->ret = SCREEN_MENU;

    return screens_quiz2_close(media, objects, var);
}
...
Question *quiz2_question_1_7_create(bool inverted) {
    ...
    if (inverted) {
        return question_create(a, b, c, text2, text1, text1, false);
    }
    return question_create(a, b, c, text1, text2, text2, false);
}
...
Question *quiz2_question_4_10_create(bool inverted) {
    ...
    sprintf(text2, "x²/4-%dx/2+%d/4", a, a*a);
...
Question *quiz2_question_13_create() {
    ...
    sprintf(text1, "x(x-%d)²", a);
    sprintf(text2, "x³-%dx²+%dx", 2*a, a*a);

    return question_create(a, b, c, text2, text1, text1, true);
}

Question *quiz2_question_n_create(int n) {
    switch(n) {
        case 0:
        return quiz2_question_1_7_create(false);
        case 1:
        return quiz2_question_2_8_create(false);
        case 2:
        return quiz2_question_3_9_create(false);
        case 3:
        return quiz2_question_4_10_create(false);
        case 4:
        return quiz2_question_5_11_create(false);
        case 5:
        return quiz2_question_6_12_create(false);
        case 6:
        return quiz2_question_1_7_create(true);
        case 7:
        return quiz2_question_2_8_create(true);
        case 8:
        return quiz2_question_3_9_create(true);
        case 9:
        return quiz2_question_4_10_create(true);
        case 10:
        return quiz2_question_5_11_create(true);
        case 11:
        return quiz2_question_6_12_create(true);
        case 12:
        return quiz2_question_13_create();
        default:
        return NULL;
    }
}
