...
bool screens_TestScreen1_loadmedia(Window *window, struct media_t *media) {
    ...
    if(!texture_load_from_text(media->tex_text, window_get_renderer(window), media->font_normal, "TELA DE TESTE 1", textColor)) return false;
...
void screens_TestScreen1_close(Button *button1, struct media_t *media) {
    button_free(button1);
...
Screen screens_TestScreen1(Window *window) {
    ...
    if (!button_change_text(button1, window_get_renderer(window), media->font_normal, "VOLTAR", text_color, false)) {
    ...
    while (!quit) {
        ...
        if (button_isselected(button1)) {
            ret = SCREEN_TITLESCREEN;
            quit = true;
        }
    }
    
    Mix_HaltMusic();
    screens_TestScreen1_close(button1, media);
    return ret;
}
